...
async def update_drive(
    ...
    """Update placement drive - Auto-set active if last_date is future"""
    conn = get_db_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="Database connection failed")
    ...
    try:
        ...
        cursor.execute("SELECT id, last_date, status FROM placement_drives WHERE id = %s", (drive_id,))
        existing = cursor.fetchone()
        if not existing:
            raise HTTPException(status_code=404, detail="Drive not found")
        
        # ✅ FIX: Agar last_date future mein hai to status 'active' set karo
        today = date.today()
        final_status = drive.status
        
        # Agar user ne status 'closed' select kiya hai to wahi rahega
        # Agar drive ki date extend ki hai to active set karo
        ...
        """, (
            ...
            drive.last_date,
            drive.allowed_branches, 
            final_status,  # ✅ Auto-set status
            drive_id
        ))
        
        conn.commit()
        logger.info(f"✅ Drive {drive_id} updated successfully with status: {final_status}")
        return {"message": "Drive updated successfully", "status": final_status}
