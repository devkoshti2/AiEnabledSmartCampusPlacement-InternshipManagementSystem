...
def add_to_email_queue(notification_id: int, user_email: str, subject: str, template_name: str, template_data: dict):
    """Notification ko email queue mein add karo - SIMPLE VERSION"""
    conn = None
    cursor = None
    try:
        ...
        conn.commit()
        logger.info(f"✅ Added to email queue: ID={cursor.lastrowid} - {subject[:50]}... -> {user_email}")
        return True
...
def process_email_queue():
    """Email queue process karo - WITH FIXED TEMPLATE MAPPING"""
    conn = None
    cursor = None
    try:
        conn = get_db_connection()
        ...
        for email in emails:
            try:
                ...
                # ✅ Get template based on type - YEH MAPPING IMPORTANT HAI
                template_name = email['template_name']
                
                if template_name == 'shortlist':
                    html = get_shortlist_email_template(**template_data)
                elif template_name == 'drive':
                    html = get_drive_announcement_template(**template_data)
                elif template_name == 'drive_update':  # ✅ YEH LINE ADD KARO
                    html = get_drive_update_template(**template_data)
                ...
                elif template_name == 'verify_otp':
                    html = get_verify_otp_template(**template_data)
                elif template_name == 'reset_otp':
                    html = get_password_reset_otp_template(**template_data)
                else:
                    logger.warning(f"⚠️ Unknown template: {template_name}")
...
def process_single_email(email_id: int) -> bool:
    ...
    try:
        ...
        logger.info(f"📨 Processing email ID {email_id} to {email['user_email']}")
        ...
        if email['template_data']:
            try:
                if isinstance(email['template_data'], str):
                    template_data = json.loads(email['template_data'])
                else:
                    template_data = email['template_data']
                logger.info(f"📦 Parsed template data: {list(template_data.keys())}")
            except json.JSONDecodeError as e:
                logger.error(f"JSON parse error: {e}")
                template_data = {'error': 'Invalid template data'}
        
        # ✅ IMPORTANT: Get the correct template
        template_name = email['template_name']
